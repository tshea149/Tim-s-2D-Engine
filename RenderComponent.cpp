...
void RenderComponentHandle::destroy()
{
	RenderEngine::getInstance()->removeRenderComponent(*this);
}

RenderComponent* const RenderComponentHandle::access()
{
	return RenderEngine::getInstance()->accessRenderComponent(*this);
}

bool RenderComponentHandle::isValid()
{
	return (index >= 0);
}
